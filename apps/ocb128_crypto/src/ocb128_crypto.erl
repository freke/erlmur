...
-export_type([crypto_state/0, key/0, nonce/0, crypto_stats/0]).
...
-doc """
Returns the current statistics from the crypto state.
""".
-spec stats(crypto_state()) -> crypto_stats().
stats(#crypto_state{stats = Stats}) ->
    Stats.
...
-spec decrypt(binary(), crypto_state()) ->
    {ok, binary(), crypto_state()}
    | {error, repeat | drop | invalid_tag, crypto_state()}.
decrypt(
    ...
    maybe
        ...
        {repeat, NewState} ->
            {error, repeat, NewState};
        {drop, NewState} ->
            {error, drop, NewState};
        invalid_tag ->
            {error, invalid_tag, State}
    end.
...
update_state_by_classification(
    {in_order, CandidateNonce},
    Nonce0,
    State = #crypto_state{history = History, stats = Stats}
) ->
    {ok, CandidateNonce, State#crypto_state{
        decrypt_iv = CandidateNonce,
        history = update_history(Nonce0, CandidateNonce, History),
        stats = add_stats(Stats, #crypto_stats{good = 1})
    }};
...
update_state_by_classification(
    ...
    {ok, CandidateNonce, State#crypto_state{
        history = update_history(Nonce0, CandidateNonce, History),
        stats = add_stats(Stats, #crypto_stats{lost = -1, late = 1})
    }};
update_state_by_classification(
    ...
    {ok, CandidateNonce, State#crypto_state{
        decrypt_iv = CandidateNonce,
        history = update_history(Nonce0, CandidateNonce, History),
        stats = add_stats(Stats, #crypto_stats{good = 1, lost = LostCount})
    }}.

add_stats(#crypto_stats{good = G1, late = La1, lost = Lo1}, #crypto_stats{good = G2, late = La2, lost = Lo2}) ->
    #crypto_stats{good = G1 + G2, late = La1 + La2, lost = Lo1 + Lo2}.
