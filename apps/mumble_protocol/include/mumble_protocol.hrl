...
-record(user,
        {id,
         ...
         texture_hash = <<>> :: binary(),
         groups = []}).
-record(registered_user, {id, name, last_seen, last_channel_id}).
-record(ping,
...
-record(udp_session,
        {ip_port :: {inet:ip_address(), inet:port_number() | pid()}, pid :: pid()}).
-record(session_record, {session_id, user_id, session_pid :: pid()}).
-record(mumble_server_conn, {id}).
-record(session, {
    id,
    session_pid,
    user,
